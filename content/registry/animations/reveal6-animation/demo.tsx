...
/* ─────────────────────────────────────────────
   reveal6-animation — Open / Source

   The widest noise wavelength of the set, then blurred by ten — the mask opens as one soft swelling blob rather than a ragged hole.
...
const IMAGE =
  "https://res.cloudinary.com/dakrfj1oh/image/upload/c_fill,w_1000,h_1000,g_auto/v1781876068/2_jogaln.jpg"

export default function Reveal6Animation() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl.add(Flip.from(state, { ease: "none", simple: true }), 0)
        .fromTo(mask, { attr: { r: "0" } }, { attr: { r: "720" } }, 0)
        .fromTo(
          image,
  ...
  return (
    <div ref={rootRef} className="r6-root">
      <style>{CSS}</style>

      <div ref={contentRef} className="r6-content">
        <div className="r6-lead">
          <img className="r6-logo" src={LOGO} alt="Zepa" draggable={false} />
        </div>

        <div className="r6-wrap">
          <div className="r6-panel">
            <div ref={titleWrapRef} className="r6-title-wrap">
              <span ref={upRef} className="r6-title r6-title--up">
                Open
              </span>
              <span ref={downRef} className="r6-title r6-title--down">
                Source
              </span>
            </div>
          </div>

          <div ref={layoutRef} className="r6-panel r6-layout">
            <svg
              className="r6-img"
              width="1000"
              height="1000"
          ...
          <feGaussianBlur in="displacement" stdDeviation="10" />
            ...
            <p className="r6-text">
              Free to use in anything you build. Read the source, fork it, take it apart — the whole registry is yours.
            </p>
          </div>
        </div>

        <div className="r6-tail" />
      </div>
    </div>
  )
}
...
.r6-root::-webkit-scrollbar { display: none; }
.r6-root *, .r6-root *::before, .r6-root *::after { box-sizing: border-box; }

.r6-content { position: relative; width: 100%; }
...
.r6-tail { height: 110vh; }

.r6-wrap { display: grid; place-items: center; grid-template-areas: "main"; }
.r6-panel { grid-area: main; display: grid; place-items: center; width: 100%; }
/* the first panel is a full screen of nothing but the two words */
.r6-wrap .r6-panel:first-child { height: 100vh; }
...
.r6-title--up { grid-area: title-up; font-style: italic; justify-self: end; align-self: start; }
.r6-title--down { grid-area: title-down; font-weight: 700; justify-self: start; align-self: end; }
...
.r6-img { grid-area: img; max-width: 100%; height: auto; aspect-ratio: 1000 / 1000; }
.r6-text {
  grid-area: text;
  margin: 0;
...
.r6-img { width: 100%; }
.r6-title--up, .r6-title--down { justify-self: end; align-self: start; }
.r6-text { max-width: 250px; justify-self: end; align-self: end; text-align: right; }
...
@media (min-width: 849px) and (max-width: 1100px) {
  .r6-img { max-width: 74.00vh !important; }   /* 74vh tall at 1.0000 */
  .r6-title { font-size: clamp(2.4rem, 12vw, 6rem); }
  .r6-text { font-size: 0.8rem; max-width: 200px; }
  .r6-layout { grid-template-columns: 1fr 44% !important; column-gap: 2.4vw; }
}
...
@media (max-width: 848px) {
  ...
  .r6-title { font-size: clamp(1.6rem, 8.5vw, 3rem); }
  .r6-title--up { justify-self: end !important; align-self: center !important; }
  .r6-title--down { justify-self: start !important; align-self: center !important; }
  .r6-img {
    width: 100% !important;
    max-width: min(76vw, 46.00vh) !important;   /* 46vh tall at 1.0000 */
    justify-self: center;
  ...
  .r6-lead { height: 10vh; }
  .r6-logo { width: min(44vw, 170px); max-height: 7vh; }
  .r6-tail { height: 70vh; }
}

@media (prefers-reduced-motion: reduce) {
  .r6-title { transition: none; }
}
`
